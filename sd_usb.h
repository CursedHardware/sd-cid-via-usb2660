...
} __attribute__((packed)) SCSI_PASS_THROUGH_DIRECT_WITH_BUFFER,
...
#define SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_R2                             \
  (SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_LENGTH_136 |                        \
   SDHC_COMMAND_REG_COMMAND_CRC_CHECK_ENABLE)
#define SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_R3_R4                          \
  (SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_LENGTH_48)
#define SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_R1_R5_R6_R7                    \
  (SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_LENGTH_48 |                         \
   SDHC_COMMAND_REG_COMMAND_INDEX_CHECK_ENABLE |                               \
   SDHC_COMMAND_REG_COMMAND_CRC_CHECK_ENABLE)
#define SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_R1b_R5b                        \
  (SDHC_COMMAND_REG_RESPONSE_TYPE_RESPONSE_LENGTH_48_CHECK_BUSY |              \
   SDHC_COMMAND_REG_COMMAND_INDEX_CHECK_ENABLE |                               \
   SDHC_COMMAND_REG_COMMAND_CRC_CHECK_ENABLE)
...
#define COMMAND_REG(_cmd_index, _flags, _response)                             \
  (((_cmd_index) << 8) | ((_response) | (_flags)))

#define SDHostCmd52(hFile, arg, response)                                      \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_52, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE),  \
            response)

#define SDHostCmd52WithData(hFile, arg, data)                                  \
  SDHostCmdWithData(                                                           \
      hFile, 0, 0, arg, 0,                                                     \
      COMMAND_REG(K_SD_CMD_52, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE), data,  \
      FALSE)

#define SDHostCmd53(hFile, block_size, block_count, arg, xfer_mode, response)  \
  SDHostCmd(hFile, block_size, block_count, arg, xfer_mode,                    \
            COMMAND_REG(K_SD_CMD_53,                                           \
                        (HC_REG_DATA_PRESENT | HC_REG_CRC_ENABLE),             \
                        HC_REG_48B_RESPONSE),                                  \
            response)

#define SDHostCmd53WithData(hFile, block_size, block_count, arg, xfer_mode,    \
                            data, write)                                       \
  SDHostCmdWithData(hFile, block_size, block_count, arg, xfer_mode,            \
                    COMMAND_REG(K_SD_CMD_53,                                   \
                                (HC_REG_DATA_PRESENT | HC_REG_CRC_ENABLE),     \
                                HC_REG_48B_RESPONSE),                          \
                    data, write)
...
#define SDHostCmd8(hFile, block_size, block_count, arg, xfer_mode, response)   \
  SDHostCmd(hFile, block_size, block_count, arg, xfer_mode,                    \
            COMMAND_REG(8, (HC_REG_DATA_PRESENT | HC_REG_CRC_ENABLE),          \
                        HC_REG_48B_RESPONSE),                                  \
            response)

#define SDHostCmd56(hFile, block_size, block_count, arg, xfer_mode, response)  \
  SDHostCmd(hFile, block_size, block_count, arg, xfer_mode,                    \
            COMMAND_REG(56, (HC_REG_DATA_PRESENT | HC_REG_CRC_ENABLE),         \
                        HC_REG_48B_RESPONSE),                                  \
            response)

#define SDHostCmd25(hFile, block_size, block_count, arg, xfer_mode, response)  \
  SDHostCmd(hFile, block_size, block_count, arg, xfer_mode,                    \
            COMMAND_REG(K_SD_CMD_25,                                           \
                        (HC_REG_DATA_PRESENT | HC_REG_CRC_ENABLE),             \
                        HC_REG_48B_RESPONSE),                                  \
            response)

#define SDHostCmd2(hFile, response)                                            \
  SDHostCmd(hFile, 0, 0, 0, 0,                                                 \
            COMMAND_REG(K_SD_CMD_2, HC_REG_CRC_ENABLE, HC_REG_136B_RESPONSE),  \
            response)

#define SDHostCmd12(hFile, response)                                           \
  SDHostCmd(                                                                   \
      hFile, 0, 0, 0, 0,                                                       \
      COMMAND_REG(K_SD_CMD_12, HC_REG_CRC_ENABLE, HC_REG_48B_BUSY_RESPONSE),   \
      response)

#define SDHostCmd9(hFile, arg, response)                                       \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_9, HC_REG_CRC_ENABLE, HC_REG_136B_RESPONSE),  \
            response)

#define SDHostCmd10(hFile, arg, response)                                      \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_10, HC_REG_CRC_ENABLE, HC_REG_136B_RESPONSE), \
            response)

#define SDHostCmd13(hFile, arg, response)                                      \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_13, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE),  \
            response)

#define SDHostCmd6(hFile, arg, response)                                       \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_6, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE),   \
            response)

#define SDHostCmd7(hFile, arg, response)                                       \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_7, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE),   \
            response)

#define SDHostCmd5(hFile, arg, response)                                       \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(K_SD_CMD_5, 0, HC_REG_48B_RESPONSE), response)

#define SDHostCmd60(hFile, arg, response)                                      \
  SDHostCmd(hFile, 0, 0, arg, 0,                                               \
            COMMAND_REG(60, HC_REG_CRC_ENABLE, HC_REG_48B_RESPONSE), response)
